...
function ProductsList() {
  ...
  const matchesCategory = (productCategory: string, selectedFilter: string) => {
    ...
    const normalizedSelectedFilter = normalizeCategoryValue(selectedFilter);

    const selectedIsParent = categoryGroups.some((group) => group.parent === selectedFilter);
    const selectedGroup = categoryGroups.find((group) => group.parent === selectedFilter);

    if (selectedIsParent && selectedGroup) {
      return (
        normalizedProductCategory === normalizeCategoryValue(selectedGroup.parent) ||
        selectedGroup.children.some((child) => normalizedProductCategory === normalizeCategoryValue(child))
      );
    }

    return normalizedProductCategory === normalizedSelectedFilter;
  };
    ...
    <div className="min-h-screen">
      ...
      <section className="mx-auto max-w-[1600px] px-4 sm:px-6 lg:px-10 pt-10 sm:pt-12 pb-16 sm:pb-20 animate-fade-in-up">
        ...
        <div className="flex flex-col items-center gap-4 mb-10 sm:mb-12">
          ...
          {/* Filtros de categorias com carrossel */}
          <div className="w-full">
            ...
            <div className="flex justify-center mb-6">
              <button
                ...
                className={`rounded-full px-8 py-3 text-base font-bold transition-all duration-300 hover:scale-105 ${
                  filter === "Todos"
            ...
            <div className="relative max-w-6xl mx-auto">
              ...
              {canScrollLeft && (
                <button
                  ...
                  className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/95 hover:bg-white shadow-xl rounded-full p-2.5 transition-all duration-300 hover:scale-110 -ml-3 border-2 border-caramel/20"
                  aria-label="Rolar para esquerda"
                >
                  <ChevronLeft className="h-5 w-5 text-caramel-deep" />
                </button>
              )}
              ...
              <div
                ...
                className="flex gap-3 overflow-x-auto scroll-smooth px-2 py-2 hide-scrollbar"
                  ...
                  return (
                    <div key={category} className="flex flex-col items-center flex-shrink-0">
                      <button
                        ...
                        className={`rounded-full px-8 py-3.5 text-base font-semibold transition-all duration-300 hover:scale-105 whitespace-nowrap ${
                          filter === category
                            ? "bg-rose text-foreground shadow-soft ring-2 ring-rose/50 scale-105"
                            : "bg-secondary/60 hover:bg-rose/40"
                        }`}
                      ...
                      {isExpanded && hasChildren && (
                        <div className="flex flex-col items-center gap-2 mt-3 w-full animate-slide-down">
                          {children.map((child, index) => (
                            <button
                              ...
                              className={`rounded-full px-4 py-2 text-xs font-medium transition-all duration-300 animate-fade-in-up-item whitespace-nowrap ${
                                filter === child
              ...
              {canScrollRight && (
                <button
                  ...
                  className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/95 hover:bg-white shadow-xl rounded-full p-2.5 transition-all duration-300 hover:scale-110 -mr-3 border-2 border-caramel/20"
                  aria-label="Rolar para direita"
                >
                  <ChevronRight className="h-5 w-5 text-caramel-deep" />
                </button>
              )}
            </div>
          </div>
